    ...
    <div class="formContainer self-center">
        ...
        <form @submit.prevent="submit">
            <p class="text-h5 text-center">
                ...
                <q-banner rounded  inline-actions :class="activationBanner" class="q-mb-sm text-center" v-if="$route.params.token">
            <q-icon style="font-size: 2em;" :name="activationIcon" /> {{ activationMessage }}
        </q-banner>
            <q-input dark v-model="$v.email.$model" label="Username/Email" @keyup.enter="login" :error="$v.email.$dirty &&!$v.email.required" error-message="Username/Email is required." bottom-slots>
            </q-input>
            <q-input dark v-model="$v.password.$model" :type="isPwd ? 'password' : 'text'" class="q-mt-sm form__input" label="Password" @keyup.enter="login" :error="$v.password.$dirty && !$v.password.required ||$v.password.$dirty && !$v.password.minLength " error-message="Password must be at least 8 and required." bottom-slots>
            ...
            <div>
                ...
                <q-btn :loading="loading" @click="login" color="primary" class="q-ma-sm" no-caps>
                    Login
                </q-btn>

            </div>
            <div class="float-left">
              <q-btn to="/password/reset" color="white" class="q-ma-sm" flat no-caps>
                    Forgot Password
                </q-btn>
            </div>
            <div class="q-mt-sm float-right">
                <v-facebook-login app-id="479157435965455"></v-facebook-login>
            </div>
      ...
      if (this.$v.$error) {
        ...
        this.$axios.post('/login', {
          ...
          .then(res => {
            ...
            if (this.prevRoute.params.token !== undefined) {
              this.$router.push('/')
            } else {
